    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->request->isPost && $order->load(Yii::$app->request->post())) {
            ...
            if($order->user_order_email){
                if(!\app\modules\email\models\EmailQueue::queue('email_marketing', $order->user_order_email, 'Order success', Yii::$app->controller->module->id, 'order_invoice', $order->getAttributes()+['detail' => $cart_cookie]))
                {
                    $transaction->rollBack();
                    Yii::$app->session->setFlash('error', 'System error');
                    return $this->render('create', [
